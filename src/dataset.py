...
class MetricDataset(Dataset):
    ...
    def __get_double_imgs_list(self):
        ...
        valid_imgs_A_path = self.__filter_valid_images(A, self.metric_pairs)
        valid_imgs_B_path = self.__filter_valid_images(B, self.metric_pairs)
        min_count = min(
            len(valid_imgs_A_path), len(valid_imgs_B_path), self.metric_pairs
        )

        return valid_imgs_A_path[:min_count], valid_imgs_B_path[:min_count]
...
class AdaptiveMetricDataset(Dataset):
    def __init__(self, config):
        ...
        self.A, self.B, self.C = self.__get_triple_imgs_list()
    ...
    def __get_triple_imgs_list(self):
        all_people = [f for f in self.root_dir.iterdir() if f.is_dir()]
        all_people = sorted(all_people)
        random.shuffle(all_people)
        ...
        valid_imgs_A_path = self.__filter_valid_images(A, self.metric_pairs)
        valid_imgs_B_path = self.__filter_valid_images(B, self.metric_pairs)
        valid_imgs_C_path = self.__filter_valid_images(C, self.metric_pairs)
        min_count = min(
