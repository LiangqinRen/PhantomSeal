...
class Lowkey(Defense):
    ...
    def _perturb_imgs_lowkey(self, imgs: Tensor, silent: bool = False) -> Tensor:
        ...
        clean_identity = self._get_imgs_identity(imgs)
        lowkey_config = self.config.third_party.lowkey
        epsilon = (
            lowkey_config.epsilon
        ...
        if not silent:
            with torch.no_grad():
                ...
                baseline_blur_identity = self._get_imgs_identity(self.blur(imgs))
        ...
        for epoch in range(lowkey_config.epochs):
            ...
            blur_identity = self._get_imgs_identity(self.blur(x_imgs))
            identity_loss = (
                lowkey_config.weight.identity
                * -(
