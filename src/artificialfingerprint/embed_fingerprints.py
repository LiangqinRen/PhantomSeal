...
parser.add_argument(
    "--encoder_path",
    type=str,
    default=os.path.join(os.path.dirname(__file__), "checkpoint", "encoder.pth"),
    help="Path to trained StegaStamp encoder.",
)
parser.add_argument(
    "--data_dir",
    type=str,
...
parser.add_argument(
    "--output_dir",
    type=str,
    default=os.path.join(os.path.dirname(__file__), "encode_samples"),
...
def load_data():
    ...
    if args.use_celeba_preprocessing:
        ...
        ), f"CelebA preprocessing requires image resolution 128, got {args.image_resolution}."
...
def embed_fingerprints():
    ...
    fingerprints = fingerprints.view(1, FINGERPRINT_SIZE).expand(
    ...
    for images, _ in tqdm(dataloader):
        ...
        if args.check:
            ...
            bitwise_accuracy += (
                (
                    detected_fingerprints[: images.size(0)].detach()
                    == fingerprints[: images.size(0)]
                )
                .float()
                .mean(dim=1)
                .sum()
    ...
    if args.check:
        ...
        save_image(
            images[:49], os.path.join(args.output_dir, "test_samples_clean.png"), nrow=7
        )
        save_image(
            fingerprinted_images[:49],
            os.path.join(args.output_dir, "test_samples_fingerprinted.png"),
            nrow=7,
        )
        save_image(
