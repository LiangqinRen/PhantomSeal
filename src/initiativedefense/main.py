...
from src.initiativedefense.defense import Defense
...
def main(config: DictConfig):
    ...
    timer = common_utils.Timer(
        f"InitiativeDefense {config.third_party.function}", logger
    )
    ...
    if config.third_party.function in defense_functions:
        ...
        sys.exit(
            f"Fail to find InitiativeDefense function: {config.third_party.function}"
        )
