...
class Lowkey(Defense):
    ...
    def metric(self) -> None:
        ...
        dataloader = DataLoader(
            dataset, batch_size=config.lowkey.batch_size, shuffle=True
        )
        total_count = 0
        ...
        for idx, (imgs_A, imgs_B) in enumerate(dataloader, start=1):
            ...
            pert_imgs = self._perturb_imgs_lowkey(
                imgs_A,
                silent=config.lowkey.silent_perturb,
            )
    ...
    def _perturb_imgs_lowkey(self, imgs: Tensor, silent: bool = False) -> Tensor:
        ...
        clean_identity = self._get_imgs_identity(self._normalize(imgs))
        ...
        if not silent:
            with torch.no_grad():
                ...
                baseline_blur_identity = self._get_imgs_identity(
                    self._normalize(self.blur(imgs))
                )
        ...
        for epoch in range(lowkey_config.epochs):
            ...
            blur_identity = self._get_imgs_identity(self._normalize(self.blur(x_imgs)))
            identity_loss = (
                lowkey_config.weight.identity
                * -(
