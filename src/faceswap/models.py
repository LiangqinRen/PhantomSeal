...
class Flatten(nn.Module):
    def forward(self, input):
        output = input.contiguous().view(input.size(0), 1024 * 4 * 4)
        return output
...
class Autoencoder(nn.Module):
    def __init__(self, decoder_count: int):
        ...
        for i in range(1, decoder_count + 1):
            self.decoders[str(i)] = nn.Sequential(
                _UpScale(512, 256),
                _UpScale(256, 128),
                _UpScale(128, 64),
    ...
    def forward(self, x, decoder_index: int):
        out = self.encoder(x)
        out = self.decoders[str(decoder_index)](out)

        return out


class ProtectAE(nn.Module):
    def __init__(self, limits):
        super(ProtectAE, self).__init__()
        ...
        self.trump_decoder = nn.Sequential(
            _UpScale(512, 256),
            _UpScale(256, 128),
            _UpScale(128, 64),
            Conv2d(64, 3, kernel_size=5, padding=1),
        ...
        self.cage_decoder = nn.Sequential(
            _UpScale(512, 256),
            _UpScale(256, 128),
            _UpScale(128, 64),
            Conv2d(64, 3, kernel_size=5, padding=1),
    ...
    def forward(self, x, decoder="trump"):
        if decoder == "trump":
            out = self.encoder(x)
            out = self.trump_decoder(out)
        else:
            out = self.encoder(x)
            out = self.cage_decoder(out)

        out = torch.clamp(out, x - self.limits, x + self.limits)
